...
def sell_puts(client, allowed_symbols, buying_power, logger = None):
    """
    ...
    logger.set_filtered_symbols(filtered_symbols)
    ...
    if logger:
        logger.log_put_options([p.to_dict() for p in put_options])
    ...
    if put_options:
        ...
        for p in put_options:
            ...
            if logger:
                logger.log_sold_puts([p.to_dict()])
    else:
        print("No put options found with sufficient delta and open interest.")

def sell_calls(client, symbol, purchase_price, stock_qty, logger = None):
    """
    ...
    if logger:
        logger.log_call_options([c.to_dict() for c in call_options])
    ...
    if call_options:
        ...
        if logger:
            logger.log_sold_calls(contract.to_dict())